...
regexp(Y, X) -> BOOL
  Supports ECMAScript-style regexp inside SQLite via the REGEXP infix operator.
  Regexp Y is not bounded by default: Partial matches are OK.
  Guard it with ^ and $ respectively to bound it.

regexp_match(Y, X, SYNTAXFLAGS, MATCHFLAGS) -> BOOL
  Same as regexp(), except you can specify SYNTAXFLAGS and MATCHFLAGS.
  Look up the NWScript function RegExpMatch for documentation on these.

regexp_replace(Y, X, FMT, SYNTAXFLAGS, MATCHFLAGS) -> BOOL
  Same as the NWScript function RegExpReplace().
